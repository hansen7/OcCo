...
import Imath, OpenEXR, argparse, array, os, shutil, numpy as np, matplotlib.pyplot as plt
...
def read_exr(exr_path, height, width):
	...
	# exception handler, usually it is all zero
	depth[depth < 0] = 0
...
if __name__ == '__main__':
	...
	parser.add_argument('--list_file', default=r'./demo_flist_normalised.txt')
	parser.add_argument('--intrinsics_file', default=r'../demo/intrinsics.txt')
	parser.add_argument('--output_dir', default=r'../demo')
	...
	for model_id in tqdm(model_list):
		...
		for i in range(args.num_scans):
			...
			try:
				...
				# print((points**2).sum(axis=1).max())
				# print((normalised_points**2).sum(axis=1).max())
				pcd = PointCloud()
