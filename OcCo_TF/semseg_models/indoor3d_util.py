...
DATA_PATH = os.path.join('../data', 'Stanford3dDataset_v1.2_Aligned_Version')
...
def point_label_to_obj(input_filename, out_filename, label_color=True, easy_view=False, no_wall=False):
    ...
    easy_view: only visualize furniture and floor
    """
...
def bbox_label_to_obj_room(input_filename, out_filename_prefix,
                           easy_view=False, permute=None, center=False, exclude_table=False):
    """ Visualization of bounding boxes.
        ...
        for i in range(point_bbox.shape[0]):
            fout.write('%f %f %f %d %d %d %d %f %f %f %f %f %f\n' %
                       (point_bbox[i, 0], point_bbox[i, 1], point_bbox[i, 2],
                        point_bbox[i, 3], point_bbox[i, 4], point_bbox[i, 5], point_bbox[i, 6],
