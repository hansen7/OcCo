...
import os, pdb, argparse, tensorflow as tf
	...
	with tf.Session() as my_sess:	
		...
		for var in var_map_target.keys():
			...
			if (var in intersect) and (var_map_source[var].shape == var_map_target[var].shape):
				...
				if verbose:
					print('%s has been restored from the pretrained %s' % (var, source_model_path))
				out_f.writelines('Restored: %s has been restored from the pretrained %s\n' % (var, source_model_path))
			else:
...
if __name__ == '__main__':
	...
	# print('\n\n================================')							
	# print('now load from the pre-trained model')							
	# print('================================\n\n')
