...
if __name__ == "__main__":
	...
	parser.add_argument("--f_name", type=str, default='ModelNet40_train_1024_middle')
	...
	df_train, num_train = lmdb_dataflow(lmdb_path=lmdb_file, batch_size=1,
										input_size=args.num_per_obj, output_size=args.num_per_obj,
										is_training=False)
	...
	hash_label = json.load(open(os.path.join(args.lmdb_path, 'keys.json')))
	...
	for i in tqdm(range(num_train)):
		...
		if num_obj_ - idx * file_per_h5 >= file_per_h5:
			...
			print(colored('%s_%s.h5 has been saved' % (args.f_name, idx), 'grey', 'on_green'))
			print('====================\n\n')
			idx += 1

	'''to deal with the remaining in the end'''
	# if (idx + 1) * file_per_h5 >= num_train // 8: # condition is always true
	h5_file = os.path.join(args.hdf5_path, '%s%d.h5' % (args.f_name, idx))
	hf = h5py.File(h5_file, 'w')
	...
	hf.close()
	print(colored('the last part has been saved into %s_%s.h5' % (args.f_name, idx), 'grey', 'on_green'))
